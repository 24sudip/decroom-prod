...
<form action="{{ route('services.update', $service->id) }}" method="POST" class="quikctech-form-wrapper border p-4 rounded-3 shadow-sm" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="row g-3">
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="title" class="form-control quikctech-input @error('title') is-invalid @enderror"
                   placeholder="ex. Floor Epoxy" value="{{ old('title', $service->title) }}" required>
            @error('title')
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="organization" class="form-control quikctech-input @error('organization') is-invalid @enderror"
                   placeholder="Enter Organization Name" value="{{ old('organization', $service->organization) }}">
            @error('organization')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="client_id" class="form-control quikctech-input @error('client_id') is-invalid @enderror"
                   placeholder="Enter user id" value="{{ old('client_id', $service->client_id) }}">
            @error('client_id')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
            ...
            @else
                <select name="vendor_id" class="form-select quikctech-select @error('vendor_id') is-invalid @enderror">
                    ...
                    @foreach($vendors as $vendorOption)
                        <option value="{{ $vendorOption->id }}" {{ old('vendor_id', $service->vendor_id) == $vendorOption->id ? 'selected' : '' }}>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="delivery_duration" class="form-control quikctech-input @error('delivery_duration') is-invalid @enderror"
                   placeholder="ex. next 3months / 45days" value="{{ old('delivery_duration', $service->delivery_duration) }}">
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="time_line" class="form-control quikctech-input @error('time_line') is-invalid @enderror"
                   placeholder="ex. Milestone (in days)" value="{{ old('time_line', $service->time_line) }}">
            @error('time_line')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="total_cost" class="form-control quikctech-input @error('total_cost') is-invalid @enderror"
                   placeholder="Tk" value="{{ old('total_cost', $service->total_cost) }}">
            @error('total_cost')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="material_cost" class="form-control quikctech-input @error('material_cost') is-invalid @enderror"
                   placeholder="Material Cost" value="{{ old('material_cost', $service->material_cost) }}">
            @error('material_cost')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="service_charge" class="form-control quikctech-input @error('service_charge') is-invalid @enderror"
                   placeholder="Tk" value="{{ old('service_charge', $service->service_charge) }}">
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="discount" class="form-control quikctech-input @error('discount') is-invalid @enderror"
                   placeholder="% = (Amount)Tk" value="{{ old('discount', $service->discount) }}">
            @error('discount')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-3">
            ...
            <select name="installment" class="form-select quikctech-select @error('installment') is-invalid @enderror">
                <option value="0" {{ old('installment', $service->installment) == 0 ? 'selected' : '' }}>No Installment</option>
                <option value="1" {{ old('installment', $service->installment) == 1 ? 'selected' : '' }}>3 Installments</option>
                <option value="2" {{ old('installment', $service->installment) == 2 ? 'selected' : '' }}>6 Installments</option>
                <option value="3" {{ old('installment', $service->installment) == 3 ? 'selected' : '' }}>12 Installments</option>
            </select>
            @error('installment')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        {{--<div class="col-md-3">
            ...
            <input type="number" step="0.01" name="advance" class="form-control quikctech-input @error('advance') is-invalid @enderror"
                   placeholder="Tk" value="{{ old('advance', $service->advance) }}">
            @error('advance')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-3">
            ...
            <input type="number" step="0.01" name="mid" class="form-control quikctech-input @error('mid') is-invalid @enderror"
                   placeholder="Tk" value="{{ old('mid', $service->mid) }}">
            @error('mid')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-3">
            ...
            <input type="number" step="0.01" name="final" class="form-control quikctech-input @error('final') is-invalid @enderror"
                   placeholder="Tk" value="{{ old('final', $service->final) }}">
            @error('final')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>--}}
        ...
        <div class="col-md-6">
            ...
            @if($service->catalog)
                <div class="mb-2">
                    ...
                    <a href="{{ asset($service->catalog) }}" target="_blank" class="btn btn-sm btn-outline-primary">
            ...
            <div class="quikctech-upload-area text-center">
                ...
                <label for="quikctech-catalog" class="quikctech-upload-label">
                    ...
                    <span class="d-block mt-2">
                        {{ $service->catalog ? 'Upload New Catalog' : 'Upload Catalog' }}
                    </span>
                </label>
            </div>
            @error('catalog')
        ...
        <div class="col-md-6">
            ...
            @if($service->attachment)
                <div class="mb-2">
                    ...
                    <a href="{{ asset($service->attachment) }}" target="_blank" class="btn btn-sm btn-outline-primary">
            ...
            <div class="quikctech-upload-area text-center">
                ...
                <label for="quikctech-attachment" class="quikctech-upload-label">
                    <i class="fa-solid fa-arrow-up-from-bracket"></i>
                    <span class="d-block mt-2">
                        {{ $service->attachment ? 'Upload New Attachment' : 'Upload Attachment' }}
                    </span>
                </label>
            </div>
            @error('attachment')
        ...
        <div class="col-md-12">
            ...
            <textarea name="note" class="form-control quikctech-textarea @error('note') is-invalid @enderror"
                      rows="5" placeholder="If you have other discussion write here, which will be part of this documentation">{{ old('note', $service->note) }}</textarea>
            @error('note')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            ...
            <select name="status" class="form-select quikctech-select @error('status') is-invalid @enderror">
                ...
                <option value="response" {{ old('status', $service->status) == 'response' ? 'selected' : '' }}>Response</option>
                <option value="on_hold" {{ old('status', $service->status) == 'on_hold' ? 'selected' : '' }}>On Hold</option>
                <option value="cancelled" {{ old('status', $service->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                <option value="complete" {{ old('status', $service->status) == 'complete' ? 'selected' : '' }}>Complete</option>
                <option value="record" {{ old('status', $service->status) == 'record' ? 'selected' : '' }}>Record</option>
            </select>
            @error('status')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="col-md-4">
            ...
            <input type="number" name="expire_duration" class="form-control quikctech-input @error('expire_duration') is-invalid @enderror" placeholder="Days" value="{{ old('expire_duration', $service->expire_duration) }}">
    ...
    <div class="d-flex justify-content-between mt-4">
        <div>
            ...
            <a href="{{ route('services.show', $service->id) }}" class="btn btn-info quikctech-btn me-2">
