...
@section('title', 'Post Service Manage')
...
<div class="row">
    <div class="col-lg-12">
        <div class="quicktech-manage-menu">
            <ul>
                <li><a href="{{ route('service-draft.create') }}" class="managemenu-active">Create Draft</a></li>
            </ul>
        </div>
    </div>
</div>
    ...
    <div class="col-lg-12">
        <div class="table-responsive">
            <table class="table quicktech-manage-table">
                ...
                <tbody>
                    @forelse($service_drafts as $service_draft)
                    <tr>
                        <td>
                            {{ ($service_drafts->perPage() * ($service_drafts->currentPage() - 1)) + $loop->iteration }}
                        </td>
                        <td>
                            <h6>{{ $service_draft->title }}</h6>
                        </td>
                        <td>
                            <div class="quicktech-manage-status">
                                Created: <strong>{{ $service_draft->created_at->format('Y-m-d H:i') }}</strong>
                                <br>
                                Delivery: <strong>{{ $service_draft->delivery_duration }}</strong>
                                <br>
                                Project Cost: <strong>৳{{ number_format($service_draft->project_cost, 2) }}</strong>
                            </div>
                        </td>
                        <td class="status-cell">
                            @if($service_draft->status == 1)
                        ...
                        <td class="text-start">
                            <div class="btn-group">
                                <a href="{{ route('service-draft.edit', $service_draft->id) }}" class="quicktech-manage-edit me-2">
                                    Edit
                                </a>
                                <form action="{{ route('service-draft.destroy', $service_draft->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-4">
                            <div class="quicktech-empty-state">
                                ...
                                <a href="{{ route('service-draft.create') }}" class="btn btn-primary">
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <div class="text-muted">
                ...
                {{ $service_drafts->total() }} services
            </div>
            <nav>
                {{ $service_drafts->links() }}
            </nav>
        </div>
        @endif
    </div>
