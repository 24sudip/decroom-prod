...
@section('title', 'Post Service Edit')
@section('content')
...
<form action="{{ route('service-draft.update', $service_draft->id) }}" method="POST" class="quikctech-form-wrapper border p-4 rounded-3 shadow-sm" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row g-3">
        <!-- Service Title -->
        <div class="col-md-4">
            ...
            <select name="service_id" class="form-control quikctech-input @error('service_id') is-invalid @enderror">
                ...
                <option value="{{ $service->id }}" {{ $service->id == $service_draft->service_id ? 'selected' : '' }}>{{ $service->title }}</option>
                @endforeach
            </select>
            @error('service_id')
        ...
        <div class="col-md-4">
            ...
            placeholder="ex. Floor Epoxy" value="{{ old('title', $service_draft->title) }}" required>
            @error('title')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="delivery_duration" class="form-control quikctech-input @error('delivery_duration') is-invalid @enderror" placeholder="ex. next 3months / 45days" value="{{ old('delivery_duration', $service_draft->delivery_duration) }}">
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="project_cost" class="form-control quikctech-input @error('project_cost') is-invalid @enderror" placeholder="Tk" value="{{ old('project_cost', $service_draft->project_cost) }}">
            @error('project_cost')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="material_cost" class="form-control quikctech-input @error('material_cost') is-invalid @enderror" placeholder="Material Cost" value="{{ old('material_cost', $service_draft->material_cost) }}">
            @error('material_cost')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="service_charge" class="form-control quikctech-input @error('service_charge') is-invalid @enderror" placeholder="Tk" value="{{ old('service_charge', $service_draft->service_charge) }}">
        ...
        <div class="col-md-2">
            ...
            <input type="number" step="0.01" name="discount" class="form-control quikctech-input @error('discount') is-invalid @enderror" placeholder="(Amount)Tk" value="{{ old('discount', $service_draft->discount) }}">
            @error('discount')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            ...
            @if($service_draft->service_video)
                <div class="mb-2">
                    <p class="mb-1">Current Video:</p>
                    <a href="{{ asset($service_draft->service_video) }}" target="_blank"
    ...
    <div class="d-flex justify-content-end mt-4">
        ...
        <button type="submit" class="btn btn-success quikctech-btn">
            <i class="fa-solid fa-paper-plane me-1"></i> Update Post
        </button>
    </div>
</form>
{{-- <script>
