    ...
    <div class="row">
      ...
      <div class="col-lg-3">
        <div class="quikctech-logo-desktop">
          <button class="btn quikctech-menu-btn" type="button" data-bs-toggle="offcanvas" data-bs-target="#quikctechOffcanvas1" aria-controls="quikctechOffcanvas1" title="{{ __('messages.menu') }}">
            <i class="fa-solid fa-bars"></i>
          </button>

          <a class="w-100" href="{{ route('home') }}">
            <img src="{{ asset('build/images/' . ($settings->logo_lg ?? 'frontend/images/logo-removebg-preview(3).png')) }}" class="w-100" alt="{{ __('messages.logo') }}">
          </a>
        </div>
      </div>
      ...
      <div class="col-lg-7">
        <form action="{{ route('frontend.searchlist') }}" method="GET">
          <div class="quikctech-search-input">
            <input type="text" name="keyword" placeholder="{{ __('messages.search_in', ['app' => 'Decroom']) }}" value="{{ request('keyword') }}">
            ...
            <button type="submit" style="border: none; background: transparent;margin-left: 16px;" title="{{ __('messages.search') }}">
              <img src="{{ asset('frontend/images/Group (3).png') }}" style="height: 40px; margin-top: 7px;" alt="{{ __('messages.search') }}">
            </button>
      ...
      <div class="col-lg-2">
        <div class="quikctech-cart-p">
          <div class="quicktech-cart-desktop position-relative">
            <a href="{{ route('cart.view') }}" class="position-relative" title="{{ __('messages.cart') }}">
                <img src="{{ asset('frontend/images/ant-design_shopping-cart-outlined.png') }}"
                     style="height: 40px;" alt="{{ __('messages.cart') }}">
                <span id="cart-count"
            ...
            <div class="dropdown quikctech-dropdown-wrapper" style="margin-top: -4px;">
                ...
                @else
                    <button class="btn quikctech-login-btn d-flex align-items-center gap-2"
                        ...
                        <span class="text-primary fw-semibold" style="font-size: 14px;">{{ __('messages.login') }}</span>
                    </button>
                @endauth
                <style>
                ...
                <div class="dropdown-menu quikctech-dropdown shadow-lg border-0 rounded-3 p-0">
                    @auth('customer')
                        ...
                        <div class="quikctech-profile d-flex align-items-center gap-2 p-3 border-bottom">
                            @if($customer->image)
                                <img src="{{ asset('storage/' . $customer->image) }}" class="rounded-circle" alt="{{ $customer->name }}" width="50" height="50" style="object-fit: cover;">
                            @else
                            ...
                            <div class="flex-grow-1">
                                <a href="{{ route('customer.profile') }}">
                                    <h6 class="mb-0 fw-semibold text-dark text-truncate" style="max-width: 180px;">{{ $customer->name }}</h6>
                                    <small class="text-muted d-block">
                                        {{ $customer->email ?? $customer->phone }}
                                    </small>
                                    <small class="text-muted">
                                        {{ __('messages.uid') }}: {{ str_pad($customer->id, 8, '0', STR_PAD_LEFT) }}
                                    </small>
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="quikctech-section border-bottom">
                            <div class="row text-center g-0">
                                <div class="col-4">
                                    <a href="{{ route('customer.profile', 'orders') }}" class="dropdown-item py-2 d-block text-decoration-none" title="{{ __('messages.orders') }}">
                                        <i class="fas fa-shopping-bag d-block mb-1 text-primary"></i>
                                        <small>{{ __('messages.orders') }}</small>
                                    </a>
                                </div>
                                <div class="col-4">
                                    <a href="{{ route('customer.profile', 'followed-vendors') }}" class="dropdown-item py-2 d-block text-decoration-none" title="{{ __('messages.wishlist') }}">
                                        <i class="fas fa-heart d-block mb-1 text-danger"></i>
                                        <small>{{ __('messages.wishlist') }}</small>
                                    </a>
                                </div>
                                <div class="col-4">
                                    <a href="{{ route('customer.profile', 'addresses') }}" class="dropdown-item py-2 d-block text-decoration-none" title="{{ __('messages.address') }}">
                                        <i class="fas fa-map-marker-alt d-block mb-1 text-success"></i>
                                        <small>{{ __('messages.address') }}</small>
                                    </a>
                                </div>
                            </div>
                        </div>
            ...
            <div class="dropdown-menu-end">
                <a href="javascript:;">
                    <div class="msg-header">
                        <p class="msg-header-title">{{ $customer->notifications()->count() }} Notifications</p>
                        {{-- <p class="msg-header-clear ms-auto"></p> --}}
                    </div>
                </a>
                <div class="header-notifications-list">
                    @forelse ($customer->unreadNotifications as $notification)
                    ...
                    @else
                        <div class="p-4 text-center">
                            ...
                            <h6 class="fw-semibold text-dark mb-2">{{ __('messages.welcome_to_app', ['app' => 'Decroom']) }}</h6>
                            <p class="text-muted small mb-3">{{ __('messages.sign_in_message') }}</p>
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <a href="{{ route('customer.register') }}" class="btn btn-outline-primary btn-sm">
        ...
        input.addEventListener("keyup", function () {
            ...
            searchTimer = setTimeout(() => {
                fetch(`{{ route('frontend.search') }}?q=${keyword}&category_id=${categoryId}`)
                    ...
                    .then(data => {
                        if (data.length === 0) {
                            box.innerHTML = `<div class="p-2 text-muted">{{ __('messages.no_results_found') }}</div>`;
...
function updateCartCount() {
    ...
    .then(resp => {
        if (!resp.ok) throw new Error('{{ __("messages.network_error") }}');
        return resp.json();
    })
    .then(data => {
    ...
    .catch(err => {
        console.error('{{ __("messages.cart_update_failed") }}:', err);
    });
}
...
document.addEventListener('click', function(e) {
  ...
  if (!id) return console.warn('{{ __("messages.cart_action_missing_id") }}');
  ...
  .then(json => {
    ...
    if (json.success === false) {
      ...
      console.warn('{{ __("messages.cart_update_failed") }}:', json.message || json);
    } else {
  ...
  .catch(err => {
    console.error('{{ __("messages.cart_update_error") }}:', err);
  });
});
</script>
